...
import 'package:commonhelpdesk/Screens/Ticket/requestDetails.dart';
...
class TicketRequestMain extends StatefulWidget {
  final dynamic getFunc;
  const TicketRequestMain({Key? key, this.getFunc}) : super(key: key);

  @override
  State<TicketRequestMain> createState() => _TicketRequestMainState();
}

class _TicketRequestMainState extends State<TicketRequestMain> {
  ...
  int? complaintIDPK, count = null;
  List trackData = [];
  int rowPerPage = 10, pageIndex = 1, initialPage = 0, toggle = 0;
  ...
  trackFunc() async {
    var fromDate = DateFormat("yyyy/MM/dd HH:mm").format(selectedFromDate);
    var toDate = DateFormat("yyyy/MM/dd HH:mm").format(selectedToDate);
    final response = await _trackControl.trackTableApi(context, rowPerPage,
        pageIndex, count, fromDate.toString(), toDate.toString());
  ...
  Future<void> requestDetails(int? val) async {
    setState(() {
      complaintIDPK = val;
    });
    final response =
    ...
    if (response.statusCode == 200) {
      if (kIsWeb) {
        widget.getFunc('requestDetails', complaintIDPK);
      } else {
        ...
        children: [
          SizedBox(
            ...
            child: Card(
                ...
                color: primaryColor,
                child: Center(
                    child: Text('Ticket Request', style: pageHeaderWeb))),
          ),
          Padding(
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        SizedBox(
                                          ...
                                          child: InkWell(
                                            onTap: () {
                                              requestDetails(trackData[index]
                                                  ['ComplaintIDPK']);
                                            },
